...
class ExceptionHandlingTest extends KernelSpec {
    ...
    def "handles uncaught exceptions with disabled global handler" () {

        when:
        // execute notebook with global exception handler disabled
        NotebookExecResult notebookResult = executeNotebook("exceptionHandling", ["DISABLE_GLOBAL_EXCEPTION_HANDLER": "true"])

        then:
        // verify the cells executed as expected up to the exception
        def cells = notebookResult.outJson.cells
